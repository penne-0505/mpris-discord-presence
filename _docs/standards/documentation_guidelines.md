...
5. intent から intent-derived invariant を抽出する。
6. QA test-plan の Test Matrix で AC / INV の確認手段を割り当てる。
7. 実装中に判断が変わった場合は Plan / Intent / QA を更新する。
8. 実装後に verification を作成し、実行コマンド・manual QA・残リスク・verdict を残す。
9. 実装済み挙動として共有が必要なら guide / reference を更新する。
10. draft / survey / plan を archive する場合は、archive checklist を満たすことを確認する。
