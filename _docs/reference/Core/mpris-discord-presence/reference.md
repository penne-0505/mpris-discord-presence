...
- installed runtime: `~/.local/share/mpris-discord-presence/venv`
- versioned runtimes: `~/.local/share/mpris-discord-presence/releases/*`
...
installerは`--system-site-packages`付きversioned venvへ現在のsourceを導入し、`venv` symlinkを最新の完全な
runtimeへ切り替える。unitはvenv内のentry pointを実行し、checkoutと`PYTHONPATH`を参照しない。直前のruntimeは
`previous-venv` symlinkからrollbackできる。configは初回だけ生成し、updateとrollbackでは保持する。
...
- runtime supportはArch Linux / EndeavourOSとDiscord公式Linux clientに限定する。
